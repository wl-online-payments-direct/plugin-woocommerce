document.addEventListener( 'DOMContentLoaded', () => {
	const bulkActionsRow = document.querySelector(
		'.wc-order-data-row.wc-order-bulk-actions'
	);
	const totalsRow = document.querySelector(
		'.wc-order-data-row.wc-order-totals-items'
	);
	const captureRow = document.getElementById( 'capture_items_container' );
	const itemsTable = document.querySelector(
		'table.woocommerce_order_items'
	);

	if ( ! bulkActionsRow || ! captureRow || ! itemsTable ) {
		return;
	}
	...
	function getPriceFormat() {
		const w =
			...
			{};
		const decimals = parseInt( w.currency_format_num_decimals, 10 );
		return {
			decimal: w.currency_format_decimal_sep || '.',
			thousand: w.currency_format_thousand_sep ?? ',',
			decimals: Number.isFinite( decimals ) && decimals >= 0 ? decimals : 2,
		};
	}

	function parsePrice( v ) {
		const { decimal, thousand } = getPriceFormat();
	...
	function formatComma( n ) {
		const { decimal, decimals } = getPriceFormat();
		return ( Number.isFinite( n ) ? n : 0 )
			.toFixed( decimals )
			.replace( '.', decimal );
	}
	...
	function updateCaptureTotals() {
		...
		const displayed =
			amountInput && manualCaptureAmount
				? parsePriceComma( amountInput.value )
				: total;

		if ( btnTotal ) {
			btnTotal.textContent = formatComma( displayed );
		}
	}

	function applyCaptureInputsToItems() {
		const rows = itemsTable.querySelectorAll(
			'tr.item, tr.shipping, tr.fee'
		);
		...
		rows.forEach( ( row ) => {
			...
			if ( qtyTd && ! qtyTd.dataset.wlOrig ) {
				qtyTd.dataset.wlOrig = qtyTd.innerHTML;
			}
			if ( ! totalTd.dataset.wlOrig ) {
				totalTd.dataset.wlOrig = totalTd.innerHTML;
			}
	...
	function restoreItemsTable() {
		itemsTable
			.querySelectorAll( 'tr.item, tr.fee, tr.shipping' )
			.forEach( ( row ) => {
				const qtyTd = row.querySelector( 'td.quantity' );
				const totalTd = row.querySelector( 'td.line_cost' );

				if ( qtyTd && qtyTd.dataset.wlOrig ) {
					qtyTd.innerHTML = qtyTd.dataset.wlOrig;
				}
				if ( totalTd && totalTd.dataset.wlOrig ) {
					totalTd.innerHTML = totalTd.dataset.wlOrig;
				}
			} );
	}
	...
	document.body.addEventListener( 'input', ( e ) => {
		...
		if ( totalInput ) {
			const raw = unit * q;
			totalInput.value = formatComma( raw );
		}
	...
	document.body.addEventListener(
		...
		( e ) => {
			...
			const amountEl = e.target.closest( '.wl-capture-amount' );
			if ( amountEl ) {
				amountEl.value = formatComma(
					parsePriceComma( amountEl.value )
				);
				updateCaptureTotals();
			}
		},
		...
		fetch( ajaxurl, {
			...
			.then( ( res ) => {
				if ( ! res.success ) {
					...
					alert(
						res.data.message || 'Capture submitted successfully.'
					);
