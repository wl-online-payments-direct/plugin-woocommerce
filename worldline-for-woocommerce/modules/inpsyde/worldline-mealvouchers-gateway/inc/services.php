...
use Inpsyde\WorldlineForWoocommerce\MealvouchersGateway\Payment\MealvouchersRequestModifier;
...
return static function (): array {
    ...
    return [
        ...
        "payment_gateway.$gatewayId.method_title" => static fn (): string =>
        __('Mealvouchers (Worldline)', 'worldline-for-woocommerce'),

        "payment_gateway.$gatewayId.title" => static fn () =>
        __('Mealvouchers', 'worldline-for-woocommerce'),

        "payment_gateway.$gatewayId.method_description" => static fn (): string =>
        __('Accept payments using Mealvouchers.', 'worldline-for-woocommerce'),
            ...
            return static function () use ($container): bool {
                try {
                    ...
                    foreach ($items as $item) {
                        $product_id = ($item instanceof WC_Order_Item_Product)
                            ? $item->get_product_id()
                            : ($item['product_id'] ?? 0);

                        $type = $wpdb->get_var(
                            $wpdb->prepare("SELECT `type` FROM `{$table}` WHERE product_id = %d", absint($product_id))
                        );

                        if (in_array($type, ['food', 'home', 'gift'], true)) {
        ...
        "payment_gateway.$gatewayId.payment_request_validator" =>
            new Alias('payment_gateways.noop_payment_request_validator'),

        "payment_gateway.$gatewayId.payment_processor" => new Factory(
            [
                'worldline_payment_gateway.hosted_checkout_url_factory',
                'worldline_payment_gateway.wc_order_factory',
                'vaulting.repository.wc.tokens.' . GatewayIds::HOSTED_CHECKOUT,
                'worldline_payment_gateway.hosted_checkout_language',
                'mealvouchers.request_modifier',
            ],
            static fn (
                HostedCheckoutUrlFactory $urlFactory,
                WcOrderBasedOrderFactoryInterface $orderFactory,
                WcTokenRepository $vaultedTokenRepository,
                ?string $languageResolver,
                MealvouchersRequestModifier $mealvoucherRequestModifier
            ): HostedPaymentProcessor => new HostedPaymentProcessor(
                $urlFactory,
                $orderFactory,
                $vaultedTokenRepository,
                $languageResolver,
                $mealvoucherRequestModifier
            )
        ),
        ...
        "mealvouchers.request_modifier" => new Constructor(
            MealvouchersRequestModifier::class,
            []
        ),

        "payment_gateway.$gatewayId.method_icon_provider" => new Factory([
            'assets.get_module_static_asset_url',
        ], static function (callable $getStaticAssetUrl): IconProviderInterface {
            /** @var string $src */
            $src = $getStaticAssetUrl(
                MealvouchersGatewayModule::PACKAGE_NAME,
                "images/mealvouchers-logo.svg"
            );
        ...
        "payment_gateway.$gatewayId.gateway_icons_renderer" => new Constructor(
            DefaultIconsRenderer::class,
            ["payment_gateway.$gatewayId.method_icon_provider"]
        ),
    ];
};
