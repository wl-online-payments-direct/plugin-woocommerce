...
return static function () : array {
    ...
    $getGatewayFormFields = static function (string $gatewayLabel, string $titlePlaceholder) : array {
        return ['enabled' => [
            ...
            'default' => 'no',
        ], 'title' => ['title' => \__('Title', 'worldline-for-woocommerce'), 'type' => 'text', 'description' => \__('Personalize the payment method title on the checkout page.', 'worldline-for-woocommerce'), 'desc_tip' => \__('If left empty, the default payment method name will be displayed on the checkout page.', 'worldline-for-woocommerce'), 'placeholder' => $titlePlaceholder]];
    };
    ...
    $payLaterServices = ["payment_gateway.{$gatewayIdPayLater}.form_fields" => static fn(): array => $getGatewayFormFields(\__('Klarna - Pay later (Worldline)', 'worldline-for-woocommerce'), \__('Klarna - Pay later', 'worldline-for-woocommerce')), "payment_gateway.{$gatewayIdPayLater}.method_title" => static fn(): string => \__('Klarna - Pay later (Worldline)', 'worldline-for-woocommerce'), "payment_gateway.{$gatewayIdPayLater}.title" => new Factory([], static function () use($gatewayIdPayLater) : string {
        $settings = (array) \get_option("woocommerce_{$gatewayIdPayLater}_settings", []);
        $custom = isset($settings['title']) && \is_string($settings['title']) ? \trim($settings['title']) : '';
        if ($custom !== '') {
            return $custom;
        }
        return \__('Klarna - Pay later', 'worldline-for-woocommerce');
    }), "payment_gateway.{$gatewayIdPayLater}.method_description" => static fn(): string => \__('Accept payments with Klarna - Pay later.', 'worldline-for-woocommerce'), "payment_gateway.{$gatewayIdPayLater}.description" => static fn(): string => '', "payment_gateway.{$gatewayIdPayLater}.order_button_text" => static fn(): ?string => null, "payment_gateway.{$gatewayIdPayLater}.payment_request_validator" => new Alias('payment_gateways.noop_payment_request_validator'), "payment_gateway.{$gatewayIdPayLater}.payment_processor" => new Factory(['worldline_payment_gateway.hosted_checkout_url_factory', 'worldline_payment_gateway.wc_order_factory', 'vaulting.repository.wc.tokens.' . GatewayIds::HOSTED_CHECKOUT, 'worldline_payment_gateway.hosted_checkout_language', "klarna.{$gatewayIdPayLater}.request_modifier"], static function (HostedCheckoutUrlFactory $hostedCheckoutUrlFactory, WcOrderBasedOrderFactoryInterface $wcOrderBasedOrderFactory, WcTokenRepository $wcTokenRepository, ?string $hostedCheckoutLanguage, PayLaterRequestModifier $payLaterRequestModifier) : HostedPaymentProcessor {
    ...
    $payLaterServicesPayIn3 = ["payment_gateway.{$gatewayIdPayLaterPayIn3}.form_fields" => static fn(): array => $getGatewayFormFields(\__('Klarna - Pay later - Pay in 3 (Worldline)', 'worldline-for-woocommerce'), \__('Klarna - Pay later (Pay in 3)', 'worldline-for-woocommerce')), "payment_gateway.{$gatewayIdPayLaterPayIn3}.method_title" => static fn(): string => \__('Klarna - Pay later - Pay in 3 (Worldline)', 'worldline-for-woocommerce'), "payment_gateway.{$gatewayIdPayLaterPayIn3}.title" => new Factory([], static function () use($gatewayIdPayLaterPayIn3) : string {
        $settings = (array) \get_option("woocommerce_{$gatewayIdPayLaterPayIn3}_settings", []);
        $custom = isset($settings['title']) && \is_string($settings['title']) ? \trim($settings['title']) : '';
        if ($custom !== '') {
            return $custom;
        }
        return \__('Klarna - Pay later (Pay in 3)', 'worldline-for-woocommerce');
    ...
    $financingServicesPayIn3 = ["payment_gateway.{$gatewayIdFinancingPayIn3}.form_fields" => static fn(): array => $getGatewayFormFields(\__('Klarna - Financing - Pay in 3 (Worldline)', 'worldline-for-woocommerce'), \__('Klarna - Financing (Pay in 3)', 'worldline-for-woocommerce')), "payment_gateway.{$gatewayIdFinancingPayIn3}.method_title" => static fn(): string => \__('Klarna - Financing - Pay in 3 (Worldline)', 'worldline-for-woocommerce'), "payment_gateway.{$gatewayIdFinancingPayIn3}.title" => new Factory([], static function () use($gatewayIdFinancingPayIn3) : string {
        $settings = (array) \get_option("woocommerce_{$gatewayIdFinancingPayIn3}_settings", []);
        $custom = isset($settings['title']) && \is_string($settings['title']) ? \trim($settings['title']) : '';
        if ($custom !== '') {
            return $custom;
        }
        return \__('Klarna - Financing (Pay in 3)', 'worldline-for-woocommerce');
    }), "payment_gateway.{$gatewayIdFinancingPayIn3}.method_description" => static fn(): string => \__('Accept payments with Klarna - Financing.', 'worldline-for-woocommerce'), "payment_gateway.{$gatewayIdFinancingPayIn3}.description" => static fn(): string => '', "payment_gateway.{$gatewayIdFinancingPayIn3}.order_button_text" => static fn(): ?string => null, "payment_gateway.{$gatewayIdFinancingPayIn3}.payment_request_validator" => new Alias('payment_gateways.noop_payment_request_validator'), "payment_gateway.{$gatewayIdFinancingPayIn3}.payment_processor" => new Alias("payment_gateway.{$gatewayIdFinancing}.payment_processor"), "payment_gateway.{$gatewayIdFinancingPayIn3}.supports" => new Alias('klarna.global.supports'), "payment_gateway.{$gatewayIdFinancingPayIn3}.refund_processor" => new Alias('payment_gateway.' . GatewayIds::HOSTED_CHECKOUT . '.refund_processor'), "payment_gateway.{$gatewayIdFinancingPayIn3}.availability_callback" => new FuncService(['klarna.global.availability_checker'], static function (PaymentGateway $paymentGateway, callable $availabilityChecker) : bool {
