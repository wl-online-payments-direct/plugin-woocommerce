...
class ProductTypeModule implements ExecutableModule, ServiceModule
{
    ...
    {
        add_meta_box(
            'worldline-product-type',
            __('Worldline Direct', 'worldline-for-woocommerce'),
    ...
    public function renderProductMetaBox($post): void
    {
        ...
        $current_type = $this->db->get_var(
            $this->db->prepare("SELECT `type` FROM `$table` WHERE product_id = %d", $product_id)
        );

        ?>
        <p><label for="worldline_mealvoucher_type">
                <?php _e('Worldline Mealvouchers Product Type:', 'worldline-for-woocommerce'); ?>
            </label></p>

        <select name="worldline_mealvoucher_type" id="worldline_mealvoucher_type" style="width: 100%;">
            <option value="none" <?php selected($current_type, null) ?>><?php _e('None', 'worldline-for-woocommerce'); ?></option>
            <option value="food" <?php selected($current_type, 'food'); ?>><?php _e('Food and drink', 'worldline-for-woocommerce'); ?></option>
            <option value="home" <?php selected($current_type, 'home'); ?>><?php _e('Home and garden', 'worldline-for-woocommerce'); ?></option>
            <option value="gift" <?php selected($current_type, 'gift'); ?>><?php _e('Gift and flowers', 'worldline-for-woocommerce'); ?></option>
        </select>

        <p style="text-align: center;">
            <button type="button" class="button button-primary" id="worldline_save_product_type">
                <?php _e('Save', 'worldline-for-woocommerce'); ?>
            </button>
        </p>
    ...
    {
        if (
            !isset($_POST['worldline_mealvoucher_nonce']) ||
            !wp_verify_nonce($_POST['worldline_mealvoucher_nonce'], 'save_worldline_mealvoucher_type')
        ) {
            return;
        }
        ...
        if (isset($_POST['worldline_mealvoucher_type'])) {
            ...
            $exists = $this->db->get_var(
                $this->db->prepare("SELECT COUNT(*) FROM `$table` WHERE product_id = %d", $post_id)
            );
    ...
    public function ajaxSaveProductType(): void
    {
        ...
        $exists = $this->db->get_var(
            $this->db->prepare("SELECT COUNT(*) FROM `$table` WHERE product_id = %d", $productId)
        );
